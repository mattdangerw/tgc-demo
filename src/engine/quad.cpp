...
void Quad::initHelper() {
  ...
  handle = theEngine().attributeHandle("tex_coord");
  glEnableVertexAttribArray(handle);
  glVertexAttribPointer(handle, 2, GL_FLOAT, GL_FALSE, 0, NULL);
}
...
void Quad::useTexture(string texture_file) {
  textured_ = true;
  texture_handle_ = theEngine().getTexture(texture_file);
}
...
void Quad::draw() {
  if (textured_) {
    ...
    glUniform4fv(theEngine().uniformHandle("color"), 1, glm::value_ptr(color_));
  }
...
void Quad::drawOccluder() {
  ...
  glUniform4fv(theEngine().uniformHandle("color"), 1, glm::value_ptr(occluder_vec));
  glUniformMatrix3fv(theEngine().uniformHandle("modelview"), 1, GL_FALSE, glm::value_ptr(fullTransform()));
  glBindVertexArray(array_object_);
  glDrawArrays(GL_TRIANGLE_FAN, 0, 4);
}
