...
void emit(vec4 p1, vec4 p2, vec4 p3, vec4 p4,
  ...
  // Order our vertex emissions so our triangles do not overlap
  if (s1 == s4) {
    gl_Position = p1;
  ...
  // Don't emit control point that are inside the hull. Drawing parts of the
  // curve twice will cut out those parts, because of the stencil trick
  if (s2 != s4 || s4 == s3) {
    gl_Position = p2;
